                        ...
                        <table class="table  table-hover" id="dataTable" width="100%" cellspacing="0">
                            ...
                            @foreach($teach_reyting as $k=>$item)
                                ...
                                <tr class="bg-primary text-white">
                                    <td style="font-size: 48px; font-weight: bolder">{{++$k}}</td>
                                    <td><a href="{{url('/user/showall',$item->id)}}"><img src="{{asset('storage/profileimages/'.$item->profileimage)}}" alt="" width="64px" style="border-radius: 45%;"></a></td>
                                    <td><a href="{{url('/user/showall',$item->id)}}">{{$item->name}}</a></td>
                                    <td>{{$item->lavozim}}</td>
                                    <td>{{$item->kafName}}</td>
                                    <td style="font-size: 48px; font-weight: bolder">{{$item->ball}}</td>
                                </tr>
                                @else
                                    <tr >
                                        <td>{{++$k}}</td>
                                        <td><a href="{{url('/user/showall',$item->id)}}"><img src="{{asset('storage/profileimages/'.$item->profileimage)}}" alt="" width="64px" style="border-radius: 45%;"></a></td>
                                        <td style="text-decoration: none"><a href="{{url('/user/showall',$item->id)}}">{{$item->name}}</a></td>
                                        <td>{{$item->lavozim}}</td>
                                        <td>{{$item->kafName}}</td>
                                        <td>{{$item->ball}}</td>
                                    </tr>
