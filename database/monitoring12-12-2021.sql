...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 12, 2021 at 07:12 AM
...
(8, 31, '1635943316_icons8-expert-128.png', '2021-10-11 05:53:28', '2021-11-03 07:41:56'),
...
(13, 39, '1635933485_rasm.jpg', '2021-10-12 06:39:30', '2021-11-03 04:58:05'),
(14, 41, '1634797111_фото А.Султонов.jpg', '2021-10-21 01:18:31', '2021-10-21 01:18:31'),
(15, 42, '1635931559_AbdulxakovYA.jpg', '2021-11-27 01:54:47', '2021-11-03 04:25:59');
...
--
-- Stand-in structure for view `v_ilmiy_reyting`
-- (See below for the actual view)
--
CREATE TABLE `v_ilmiy_reyting` (
`id` bigint(20) unsigned
,`kafid` int(11)
,`kafName` varchar(255)
,`name` varchar(191)
...
--
-- Stand-in structure for view `v_pedagogik_reyting`
-- (See below for the actual view)
--
CREATE TABLE `v_pedagogik_reyting` (
`id` bigint(20) unsigned
,`kafid` int(11)
,`kafName` varchar(255)
,`name` varchar(191)
...
--
-- Structure for view `v_ilmiy_reyting`
--
DROP TABLE IF EXISTS `v_ilmiy_reyting`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ilmiy_reyting`  AS SELECT `u`.`id` AS `id`, `u`.`kafid` AS `kafid`, `kaf`.`kafName` AS `kafName`, `u`.`name` AS `name`, `lavozim`.`lavozim` AS `lavozim`, ifnull(`dr`.`ball`,0) + ifnull(`loyiha`.`ball`,0) + ifnull(`maqola`.`ball`,0) + ifnull(`patent`.`ball`,0) AS `ball` FROM ((((((`v_teachers` `u` left join `v_diss_reyting` `dr` on(`u`.`id` = `dr`.`id`)) left join `v_loyiha_reyting` `loyiha` on(`loyiha`.`id` = `u`.`id`)) left join `v_maqola_reyting` `maqola` on(`maqola`.`id` = `u`.`id`)) left join `v_patent_reyting` `patent` on(`patent`.`id` = `u`.`id`)) left join `kafedras` `kaf` on(`kaf`.`id` = `u`.`kafid`)) left join `lavozims` `lavozim` on(`lavozim`.`id` = `u`.`lavozimid`)) GROUP BY `u`.`id`, `u`.`kafid`, `u`.`name`, `kaf`.`kafName`, `lavozim`.`lavozim` ORDER BY ifnull(`dr`.`ball`,0) + ifnull(`loyiha`.`ball`,0) + ifnull(`maqola`.`ball`,0) + ifnull(`patent`.`ball`,0) DESC ;
...
--
-- Structure for view `v_pedagogik_reyting`
--
DROP TABLE IF EXISTS `v_pedagogik_reyting`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_pedagogik_reyting`  AS SELECT `u`.`id` AS `id`, `u`.`kafid` AS `kafid`, `kaf`.`kafName` AS `kafName`, `u`.`name` AS `name`, `lavozim`.`lavozim` AS `lavozim`, ifnull(`darsr`.`ball`,0) + ifnull(`kurs`.`ball`,0) + ifnull(`lang`.`ball`,0) + ifnull(`malaka`.`ball`,0) + ifnull(`material`.`ball`,0) AS `ball` FROM (((((((`v_teachers` `u` left join `v_dars_reyting` `darsr` on(`darsr`.`id` = `u`.`id`)) left join `v_kurs_reyting` `kurs` on(`kurs`.`id` = `u`.`id`)) left join `v_lang_reyting` `lang` on(`lang`.`id` = `u`.`id`)) left join `v_malaka_reyting` `malaka` on(`malaka`.`id` = `u`.`id`)) left join `v_material_reyting` `material` on(`material`.`id` = `u`.`id`)) left join `kafedras` `kaf` on(`kaf`.`id` = `u`.`kafid`)) left join `lavozims` `lavozim` on(`lavozim`.`id` = `u`.`lavozimid`)) GROUP BY `u`.`id`, `u`.`kafid`, `u`.`name`, `kaf`.`kafName`, `lavozim`.`lavozim` ORDER BY ifnull(`darsr`.`ball`,0) + ifnull(`kurs`.`ball`,0) + ifnull(`lang`.`ball`,0) + ifnull(`malaka`.`ball`,0) + ifnull(`material`.`ball`,0) DESC ;
